...
async def start_experiment(config: ExperimentConfiguration):
    """
    ...
    """
    try:
        result = await experiment_service.create_experiment(config)
        return result
...
async def list_experiments():
    ...
    - configuration and results (if completed)
    """
    try:
...
async def get_experiment_status(experiment_id: str):
    ...
    """
    try:
        status = experiment_service.get_experiment_status(experiment_id)
        if not status:
    ...
    try:
        ...
        if not results:
            ...
            if not status:
                ...
                raise HTTPException(status_code=400, detail=f"Experiment is {status.status}, results not available")
            else:
...
async def stop_experiment(experiment_id: str):
    ...
    """
    try:
        success = await experiment_service.stop_experiment(experiment_id)
        if not success:
            status = experiment_service.get_experiment_status(experiment_id)
            if not status:
                raise HTTPException(status_code=404, detail="Experiment not found")
            else:
                raise HTTPException(status_code=400, detail=f"Cannot stop experiment with status: {status.status}")
