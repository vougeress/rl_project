...
- **Database migrations**: The project uses SQLAlchemy declarative models. When schema changes occur, recreate the Postgres volume (`docker compose down -v` followed by `docker compose up -d`) or run manual `ALTER TABLE` statements.
- **Agents**: Add new policies in `src/agents/` and register them in `src/agents/factory.py`.
- **Environment tuning**: Modify product/user generators in `src/data_generation.py` or reward logic in `src/environment.py`. Restart the API (`docker compose restart api`) to reinitialise the global learning manager and regenerate catalog data.
- **Testing**: No formal test suite is included yet. Recommended workflow is to launch experiments through the dashboard and monitor API logs (`docker compose logs -f api`) to verify learning behaviour.

## Troubleshooting

- **Docker daemon / socket errors**: Ensure Docker Desktop is running and you have permission to access `/Users/<you>/.docker/run/docker.sock`.
- **Port conflicts**: If `5432` is taken, update the host mapping in `docker-compose.yml` (e.g., `"5433:5432"`) and adjust your client connections.
- **Missing dependencies**: Rebuild the API image after editing `requirements.txt` (`docker compose build api`).
- **Dashboard cannot reach API**: Set `API_BASE_URL` appropriately (`http://localhost:8000` when running outside Docker). The dashboard container already uses `http://api:8000`.

---

Enjoy experimenting with RL strategies for your virtual storefront! Contributions, extensions, and performance tweaks are welcome.
